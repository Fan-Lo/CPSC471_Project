...
class Patient:
    ...
    def addNewInvoice(self, invoiceID, invoiceDate, products, services):
        newInvoice = Invoice(invoiceID, invoiceDate,False)
        self.database = DatabaseConnect()
        self.database.insert(f"INSERT INTO INVOICE VALUES ({invoiceID}, '{self.__ahcNum}', '{invoiceDate}');")
        self.database.close()
        for i in products:
            #addProduct adds it to the database too
            newInvoice.addProduct(i, self.__ahcNum)

        self.__invoice.append(newInvoice)
